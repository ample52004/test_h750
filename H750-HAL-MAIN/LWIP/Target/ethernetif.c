...
rt_sem_t RxPktSemaphore = NULL;   /* Semaphore to signal incoming packets */
rt_sem_t TxPktSemaphore = NULL;   /* Semaphore to signal transmit packet complete */
...
static void low_level_init(struct netif *netif)
{
  ...
  RxPktSemaphore = rt_sem_create("RxPktSemaphore", 1, RT_IPC_FLAG_PRIO);

  /* create a binary semaphore used for informing ethernetif of frame transmission */
  TxPktSemaphore = rt_sem_create("TxPktSemaphore", 1, RT_IPC_FLAG_PRIO);

  /* create the task that handles the ETH_MAC */
/* USER CODE BEGIN OS_THREAD_NEW_CMSIS_RTOS_V2 */
  rt_thread_t tid = rt_thread_create("Ethif",ethernetif_input,netif,1024,8,10);
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  while(rt_sem_take(TxPktSemaphore, RT_WAITING_FOREVER)!=RT_EOK)
...
void ethernetif_input(void* argument)
{
  ...
  for( ;; )
  {
    if (rt_sem_take(RxPktSemaphore, RT_WAITING_FOREVER) == RT_EOK)
    {
      do
      {
