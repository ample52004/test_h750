...
void NAND_check(void)
{
    ...
    rt_kprintf("HAL_id = 0x%02x%02x%02x%02x\r\n", id.Device_Id,id.Fourth_Id,id.Maker_Id,id.Third_Id);

		addr.Block = 0;
		addr.Page = 0;
		addr.Plane = 0;
		rt_kprintf("3.HAL_NAND_Erase_Block = %d\r\n", HAL_NAND_Erase_Block(&NAND_Handler, &addr));

		memset(buf,0,2048);


    rt_kprintf("2.HAL_NAND_Read_Page_8b = %d\r\n", HAL_NAND_Read_Page_8b(&NAND_Handler, &addr, buf, 1));
    for(int i = 0; i < 2048; i++)
    {
        if((i % 5) == 0)  rt_kprintf("\r\n");
        rt_kprintf("buf[%04d] = 0x%02X ", i, buf[i]);
    }
    rt_kprintf("\r\n");
