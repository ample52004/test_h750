...
#define DBG_ENABLE
#define DBG_SECTION_NAME    "USB"
#define DBG_LEVEL           DBG_LOG
#define DBG_COLOR
#include <dbg.h>
extern struct fx_lx_nand_driver_instance *current_driver;
#define NAND_LUN_NBR                  1
#define NAND_BLK_NBR                  0x10000
#define NAND_BLK_SIZ                  0x800
...
int8_t STORAGE_GetCapacity_HS(uint8_t lun, uint32_t *block_num, uint16_t *block_size)
{
  ...
  *block_num  = NAND_BLK_NBR;
  *block_size = NAND_BLK_SIZ;
  return (USBD_OK);
  /* USER CODE END 10 */
}
...
int8_t STORAGE_Read_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  ...
  UNUSED(blk_len);
	...
	{
		status += lx_nand_flash_sector_read(&current_driver->flash_instance, blk_addr, buf);
		blk_addr ++;
		buf += 2048;
	}
...
int8_t STORAGE_Write_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  ...
  UNUSED(blk_len);
	...
	for(int i=0; i<blk_len; i++)
	{
		status += lx_nand_flash_sector_write(&current_driver->flash_instance, blk_addr, buf);
		blk_addr ++;
		buf += 2048;
	}
